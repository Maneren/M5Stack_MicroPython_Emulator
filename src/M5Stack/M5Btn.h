...
public:
  ...
  static constexpr const char *Get_Class_Name() { return "M5Btn"; }
...
public:
  ...
  M5Btn(py::str text, int x, int y, int w, int h) {
    ...
    mObj =
        CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Button>(
            txt, x, y, w, h);
  }
  ...
  M5Btn(py::str text, int x, int y, int w, int h, int textColor, int bgColor) {
    ...
    mObj =
        CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Button>(
            txt, x, y, w, h, textColor, bgColor);
  }

  void pressed(PyObject *cb) { mObj->Register_Pressed_Callback(cb); }

  void released(PyObject *cb) { mObj->Register_Released_Callback(cb); }

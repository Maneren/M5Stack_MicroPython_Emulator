...
class M5Checkbox {
	...
	public:
		...
		M5Checkbox(py::str text, int x, int y, int w, int h) {
			...
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Checkbox>(txt, x, y, w, h);
		}
		...
		M5Checkbox(py::str text, int x, int y, int w, int h, int textColor) {
			...
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Checkbox>(txt, x, y, w, h, textColor);
		}

		M5Checkbox(py::str text, int x, int y, int w, int h, int textColor, int checkColor) {

			std::string txt = py::extract<std::string>(text);

			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Checkbox>(txt, x, y, w, h, textColor, checkColor);
		}

		void checked(PyObject* cb) {
			mObj->Register_Checked_Callback(cb);
		}

		void unchecked(PyObject* cb) {
			mObj->Register_Unchecked_Callback(cb);
		}

		void set_checked(bool state) {
			mObj->Set_Checked(state);
		}

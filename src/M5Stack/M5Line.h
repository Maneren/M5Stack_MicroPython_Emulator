...
class M5Line {
	...
	public:
		...
		M5Line(int type, int x1, int y1, int x2, int y2) {
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Line>(x1, y1, x2, y2);
		}

		M5Line(int type, int x1, int y1, int x2, int y2, int color) {
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Line>(x1, y1, x2, y2, color);
		}

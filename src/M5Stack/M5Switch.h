...
class M5Switch {
	...
	public:
		...
		M5Switch(int x, int y, int w, int h) {
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Switch>(x, y, w, h);
		}

		M5Switch(int x, int y, int w, int h, int bgColor) {
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Switch>(x, y, w, h, bgColor);
		}

		M5Switch(int x, int y, int w, int h, int bgColor, int color) {
			mObj = CM5Stack_VM::Instance()->Get_Screen()->Add_Drawable<CDrawable_Switch>(x, y, w, h, bgColor, color);
		}

		void on(PyObject* cb) {
			mObj->Register_On_Callback(cb);
		}

		void off(PyObject* cb) {
			mObj->Register_Off_Callback(cb);
		}

		bool get_state() {
			return mObj->Get_State();
		}

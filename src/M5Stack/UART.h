...
class UART {
	public:
		...
		static auto&& Define_Class_Contents(auto&& obj) {
			...
			return obj.def(py::init<int, int, int>())//, py::args("channel", "tx", "rx")) //TODO: implement this somehow, Boost does not seem to support it
	...
	private:
		// selected UART channel - defaults to 1 as it is the first user available UART
	...
	public:
		...
		py::object read() {
			...
			return py::object(py::handle<>( PyBytes_FromString(CM5Stack_VM::Instance()->Get_UART()->Read(mChannel, true).c_str()) ));
		}

		py::object read(int count) {
			// convert to byte array as MicroPython expects it
			return py::object(py::handle<>( PyBytes_FromString(CM5Stack_VM::Instance()->Get_UART()->Read(mChannel, true, count).c_str()) ));
		}

		py::object readLine() {
			// convert to byte array as MicroPython expects it
			return py::object(py::handle<>( PyBytes_FromString(CM5Stack_VM::Instance()->Get_UART()->Read_Line(mChannel, true).c_str()) ));
		}

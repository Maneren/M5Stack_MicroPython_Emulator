...
CDrawable_Button::CDrawable_Button(const std::string& text, int x, int y, int width, int height, int textColor, int brushColor)
	: IDrawable(x, y), mWidth(width), mHeight(height), mText_Color(textColor), mBrush_Color(brushColor), mText(text) {
...
void CDrawable_Button::Draw(QPainter& painter) {
	...
	painter.drawText(QPoint(mPosX + mWidth / 2 - w / 2, mPosY + 2 * h / 7 + mHeight / 2), txt);
...
CDrawable_Checkbox::CDrawable_Checkbox(const std::string& text, int x, int y, int width, int height, int textColor, int checkColor)
	: IDrawable(x, y), mWidth(width), mHeight(height), mText_Color(textColor), mCheck_Color(checkColor), mText(text) {
...
bool CDrawable_Checkbox::Touched(int x, int y, bool touch) {
	if (x >= mPosX && x < mPosX + (mWidth > 0 ? mWidth : 16) && y >= mPosY && y <= mPosY + (mHeight > 0 ? mHeight : 16)) {
...
CDrawable_Switch::CDrawable_Switch(int x, int y, int width, int height, int bgColor, int switchColor)
	: IDrawable(x, y), mWidth(width), mHeight(height), mBg_Color(bgColor), mColor(switchColor) {
...
bool CDrawable_Switch::Get_State() const {
	return mIs_Checked;
}
...
void CDrawable_Switch::Draw(QPainter& painter) {
	...
	painter.drawRect(mPosX + 2 + (mIs_Checked ? ((mWidth - 2) / 2) : 0), mPosY + 2, (mWidth - 4) / 2, mHeight - 4);
}

...
bool CM5LCDScreen::Is_Changed() const {
	return mChanged;
}

void CM5LCDScreen::Clear_Changed_Flag() {
	mChanged = false;
}
...
int CM5LCDScreen::Get_Touch_X() const {
	return mTouch_X;
}

int CM5LCDScreen::Get_Touch_Y() const {
	return mTouch_Y;
}
...
bool CM5GPIO::Is_Changed() const {
	return mChanged;
}

void CM5GPIO::Clear_Changed_Flag() {
	mChanged = false;
}
...
bool CM5Buttons::isPressed(Button btn) const {
	return Btn(btn).state;
}

bool CM5Buttons::isReleased(Button btn) const {
	return !Btn(btn).state;
}
...
std::string CM5UART::Read(Channel c, bool deviceSide, int maxCnt) {
	...
	while (!fifo.empty() && (maxCnt <= 0 || r.size() < static_cast<size_t>(maxCnt))) {

...
private:
  ...
  auto &Btn(Button btn) { return mButtons[static_cast<size_t>(btn)]; }

  auto &Btn(Button btn) const { return mButtons[static_cast<size_t>(btn)]; }
...
public:
  ...
  static constexpr size_t UART_Channel_Count =
      static_cast<size_t>(Channel::count);
...
private:
  ...
  auto &FIFO(Channel c, bool tx) {
    return tx ? mTX_FIFOs[static_cast<size_t>(c)]
              : mRX_FIFOs[static_cast<size_t>(c)];
  }

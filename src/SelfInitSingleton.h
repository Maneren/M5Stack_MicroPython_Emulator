...
protected:
  ...
  Self_Init_Singleton() noexcept {
    if constexpr (!Alloc_If_Not_Existent) {
      static_assert(std::is_base_of_v<Self_Init_Singleton<T>, T>,
                    "T should derive from Self_Init_Singleton via CRTP");
    }
...
template <class T, bool Alloc_If_Not_Existent>
T *Self_Init_Singleton<T, Alloc_If_Not_Existent>::mInstance = nullptr;
